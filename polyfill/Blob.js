...
export default class Blob extends EventTarget {
  ...
  static setLog(level: number) {
    if (level === -1) log.disable();
    else log.level(level);
  }
  ...
  constructor(data: any, cType: any, defer: boolean) {
    ...
    if (data.isRNFetchBlobPolyfill) {
      ...
      p = fs.exists(orgPath).then((exist) => {
        if (exist)
          return fs
            .writeFile(orgPath, data, "uri")
    ...
    // process FormData
      ...
      if (!parts) {
        ...
        for (let i in parts) {
          ...
          formArray.push("\r\n");
          if (part.isRNFetchBlobPolyfill) formArray.push(part);
          else formArray.push(part.string);
        }
    ...
    // Blob data from file path
      ...
      typeof data === "string" &&
      data.startsWith("RNFetchBlob-file://")
    ) {
    ...
    // content from variable need create file
      ...
      if (/(application\/octet|\;base64)/i.test(mime)) encoding = "base64";
      else data = data.toString();
      // create cache file
      this.type = String(this.type).replace(/;base64/gi, "");
      log.verbose("create Blob cache file from string", "encode", encoding);
      p = fs.writeFile(this._ref, data, encoding).then((size) => {
  ...
  onCreated(fn: () => void): Blob {
    ...
    if (!this._blobCreated) this._onCreated.push(fn);
    else {
      fn(this);
    }
    return this;
