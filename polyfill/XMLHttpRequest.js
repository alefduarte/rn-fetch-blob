...
export default class XMLHttpRequest extends XMLHttpRequestEventTarget {
  ...
  static binaryContentTypes: Array<string> = ["image/", "video/", "audio/"];
  ...
  getResponseHeader(field: string): string | null {
    ...
    return (
      this._responseHeaders[field] ||
      this._responseHeaders[field.toLowerCase()] ||
      null
    );
  }
  ...
  _onError = (err) => {
    ...
    if (
      (err && String(err.message).match(/(timed\sout|timedout)/)) ||
      this._status == 408
    ) {
